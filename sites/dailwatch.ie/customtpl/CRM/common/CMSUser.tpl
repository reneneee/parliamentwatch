...
{if $showCMS }{*true if is_cms_user field is set *}
   <fieldset class="crm-group crm_user-group">
	 ...
	 {if !$isCMS}
	    {ts}You have already registered with abgeordnetenwatch.de? Then log in to our member domain with your personal access data{/ts}
	 {else}
	    {ts}Please enter a user name to create an account.{/ts}
	 {/if}
	 {ts 1=$loginUrl}<a href='%1'>go to log in</a> .{/ts}
      </div>
